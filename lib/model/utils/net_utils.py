...
def mmd(x, y):
    ...
    return mix_rbf_mmd2(x, y, [GAMMA])
    #return mmd

def new_minmaxscaler(data):
    middle = int(round(len(data)/2))
    x = data[middle]
    x = check(x)
    data = data * math.pow(10, x+1)
    max_1 = torch.max(data) + 2
    min_1 = torch.min(data) - 2
    d = max_1 - min_1 
    k = (6-4) / d
    data = 4 + k * (data - min_1)
    #data = list(data)
...
def minmaxscaler(data1,data2,data3):
    x = check(data2)
    data1 = data1 * math.pow(10, x+1)
    data2 = data2 * math.pow(10, x+1)
    data3 = data3 * math.pow(10, x+1)
    data = torch.tensor([data1,data2,data3],requires_grad=True).cuda()
    #data = data.reshape(-1,1)
    #min_max_scaler = preprocessing.MinMaxScaler()
    #data = torch.from_numpy(min_max_scaler.fit_transform(data)).type(torch.FloatTensor).cuda()
    min = torch.min(data)
    min = min - 0.2
    max = torch.max(data)
    max = max + 0.2
    ...
    #    d = 0.00001
    #data_std = (data - min) / (d)
    #data_scale = (data_std) / 1 + 0
    #print(data_scale)

    #min = torch.min(data_scale)
    #max = torch.max(data_scale)
    d = max - min
    if d == 0:
      d = 0.01
    
    k = (6-2) / d
    normal_data1 =  (2 + k * (data1 - min))
    normal_data2 =  (2 + k * (data2 - min))
    normal_data3 =  (2 + k * (data3 - min))
    gamma1 = 6 - normal_data1
    gamma2 = 6 - normal_data2
    gamma3 = 6 - normal_data3 
    #return gamma1, gamma2, gamma3
    return normal_data1,normal_data2,normal_data3
