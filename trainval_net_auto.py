...
if __name__ == '__main__':
	...
	mmd_feat3_record = []
	    ...
	    mmd_ori_avg = (mmd_feat1_ori + mmd_feat2_ori + mmd_feat3_ori) / 3.0
            mmd_avg_value.append(copy.deepcopy(mmd_ori_avg.data))
	    ...
	    FL1 = FocalLoss(class_num=2, gamma = float(mmd_1_scale[-1]))
            FL2 = FocalLoss(class_num=2, gamma = float(mmd_2_scale[-1]))
            FL3 = FocalLoss(class_num=2, gamma = float(mmd_3_scale[-1]))
	    ...
	    _, pre_domain_s = torch.max(F.softmax(domain_p_feat3_s), 1)
            score_s = torch.max(F.softmax(domain_p_feat3_s))
            
	    
            if pre_domain_s != domain_s :
		if score_s < 0.6:
	            error_count += 1
	    ...
	    _, pre_domain_t = torch.max(F.softmax(domain_p_feat3_t), 1)
	    score_t = torch.max(F.softmax(domain_p_feat3_t))
	   
	    if pre_domain_t != domain_t:
		if score_t < 0.6:
	            error_count += 1   

            loss  += ( dloss_s_feat1 + dloss_s_feat2 + dloss_s_feat3 + dloss_t_feat1 + dloss_t_feat2 + dloss_t_feat3 + 0.25 * mmd_ori_avg)
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
		    ...
		    mmd_ori_avg = mmd_ori_avg.item()
                ...
                print(
                    ...
                    % (loss_rpn_cls, loss_rpn_box, loss_rcnn_cls, loss_rcnn_box, \
                        ...
                        dloss_t_feat1, dloss_t_feat2, dloss_t_feat3, mmd_ori_avg,\
                       args.eta))
                if args.use_tfboard:
                    info = {
                        'loss': loss_temp,
	...
	mmd_record.sort()
        ...
        alpha.append(copy.deepcopy(mmd_record[rank]))
	print(alpha)
	...
	print(feat3_avg)
        save_name = os.path.join(output_dir,
                                 '{}_epoch_{}_step_{}.pth'.format(
                                     args.dataset_t,epoch,
                                     step))
        save_checkpoint({
